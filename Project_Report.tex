...
\section{Screenshots}
Add screenshots from a live simulation run in these exact states:
\begin{itemize}[leftmargin=*]
    \item \textbf{Dashboard before start}: show the full app with the sidebar, process table, CPU panel, ready queue, RAM, Gantt chart, completed list, and metrics all visible before clicking Start.
    \item \textbf{Gantt chart in progress}: capture the schedule after a few time slices have executed so the chart contains multiple blocks and at least one active block.
    \item \textbf{CPU activity}: capture the CPU panel while a process is actively running so the current PID and remaining time are visible.
    \item \textbf{Queue and memory state}: capture the ready queue and RAM panels together while several processes are waiting or resident in memory.
    \item \textbf{Metrics after completion}: capture the performance metrics section after the run finishes so all values are populated.
    \item \textbf{Adaptive feedback}: capture the sidebar feedback message after running a workload that triggers a recommendation.
    \item \textbf{Priority aging evidence}: capture the process table while priority aging is enabled and the effective-priority column is visible.
    \item \textbf{Algorithm switch banner}: capture the yellow banner that appears after changing the algorithm after a run and before resetting.
\end{itemize}

\subsection{Where To Capture Each Screenshot}
Use the following capture order during a demo session:
\begin{enumerate}[leftmargin=*]
    \item Start with a clean dashboard before the simulation begins. This is your overview screenshot.
    \item Run the simulator with a small workload and capture the Gantt chart once at least one or two blocks have appeared.
    \item Pause or observe the run while a process is on the CPU, then capture the CPU panel.
    \item Capture the ready queue and RAM after multiple processes have arrived so the resource panels look populated.
    \item Let the simulation finish and capture the metrics panel with final values filled in.
    \item Run a workload that produces a recommendation, such as FCFS with mixed burst times or priority scheduling with a starvation risk, then capture the feedback message.
    \item If you use priority aging, capture the table while the effective-priority column is shown so the aging behavior is documented.
    \item Change the algorithm after a run and capture the banner that tells you to reset and start again.
\end{enumerate}

\subsection{Screenshot Placeholders}
Replace each box below with the matching live screenshot file before final submission.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{screenshots/main-dashboard-before-start.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{screenshots/gantt-during-run.png}
    \caption{Gantt Chart During Execution, showing the live schedule after a few time slices.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{screenshots/cpu-active-process.png}
    \caption{CPU Panel While Running, showing the currently active process and its remaining time.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{screenshots/ready-queue-and-ram.png}
    \caption{Ready Queue and RAM, showing processes waiting and residing in memory during execution.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{screenshots/performance-metrics-final.png}
    \caption{Performance Metrics After Completion, displaying final calculated values for the executed workload.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{screenshots/adaptive-feedback.png}
    \caption{Adaptive Feedback Message, providing an algorithm recommendation based on the executed workload.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{screenshots/priority-aging-table.png}
    \caption{Priority Aging in Process Table, showing dynamically updated effective-priority values to prevent starvation.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{screenshots/algorithm-change-banner.png}
    \caption{Algorithm Change Banner, prompting the user to reset the simulation after switching algorithms.}
\end{figure}
